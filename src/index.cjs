...
const { useMouse } = require("./useMouse.cjs");
const { useKeyboard } = require("./useKeyboard.cjs");
...
const createWindow = async () => {
  ...
  mainWindow.webContents.session.setPreloads([
    path.join(__dirname, "preload-get-display-media-polyfill.cjs"),
  ]);
  ...
  ipcMain.handle("openDialog", (e, sources) => {
    ...
    dialogWindow.webContents.session.setPreloads([
      path.join(__dirname, "preload-get-display-media-polyfill.cjs"),
    ]);
    dialogWindow.webContents.session.setPermissionCheckHandler(
      async (webContents, permission, details) => {
        return true;
      }
    );
    dialogWindow.webContents.session.setPermissionRequestHandler(
      async (webContents, permission, callback, details) => {
        callback(true);
      }
    );
