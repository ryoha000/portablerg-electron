...
class Pan extends Gesture {
   ...
   * @param {Array} inputs
   */
  // @ts-expect-error ts-migrate(2416) FIXME: Type 'void' is not assignable to type 'null'.
  start(inputs: any) {
   ...
   */
  // @ts-expect-error ts-migrate(2416) FIXME: Type '{ data: never[]; }' is not assignable to typ... Remove this comment to see the full error message
  move(inputs: any, state: any, element: any) {
    ...
    inputs.forEach( (input: any, index: any) => {
      ...
      if (progress.active && reachedThreshold) {
        // @ts-expect-error ts-migrate(2322) FIXME: Type '{ distanceFromOrigin: number; currentDistanc... Remove this comment to see the full error message
