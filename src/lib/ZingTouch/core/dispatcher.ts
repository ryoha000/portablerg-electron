 ...
 */
function dispatcher(binding: any, data: any, events: any) {
  data.events = events;
 ...
 * @param {Binding} binding - An object of type Binding
 */
function emitEvent(target: any, event: any, binding: any) {
  target.dispatchEvent(event);
  if (binding.bindOnce) {
    // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name 'ZingTouch'.
